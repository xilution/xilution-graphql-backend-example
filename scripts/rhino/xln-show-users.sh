...
curl -s \
  ...
  "https://${environment}.rhino.basics.api.xilution.com/organizations/${sub_organization_id}/users?page-number=0&page-size=25" |
  jq '.content | if (. | length > 0) then (.[] | {id: .id, username: .username}) else "No users found." end'
