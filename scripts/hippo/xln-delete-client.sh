...
if [[ ! ${1} =~ ${XILUTION_ID_REGEX} ]]; then
  echo "Client ID (${1}) should match the pattern: ${XILUTION_ID_REGEX}"
  exit 1
fi
...
curl -s \
  ...
  "https://${environment}.hippo.basics.api.xilution.com/organizations/${sub_organization_id}/clients/${client_id}"
